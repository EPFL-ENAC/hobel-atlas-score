...
const colors = computed(() => {
  ...
  try {
    ...
    if (!props.schemeName.startsWith('interpolate')) {
      // @ts-expect-error D3 color schemes are accessed dynamically
      const scheme = (d3 as Record<string, Record<number, string[]>>)[props.schemeName];
    ...
    // @ts-expect-error D3 interpolators are accessed dynamically
    const interpolator = (d3 as Record<string, (t: number) => string>)[interpolateName];
