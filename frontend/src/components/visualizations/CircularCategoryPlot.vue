...
const calculateCategoryAverages = (data: EnvironmentalData[]) => {
  ...
  categoryOrder.forEach((category) => {
    ...
    if (categoryData && categoryData.length > 0) {
      ...
      const color: string = colors[bandIndex] ?? colors[colors.length - 1] ?? '#ccc';
...
const createCircularPlot = () => {
  ...
  slices
    ...
    .on('mouseover', function (event, d) {
      ...
      const numBands = props.numBands || 4;
      const bandSize = 100 / numBands;
      const minRange = String(Math.round(d.data.band * bandSize));
      const maxRange = String(Math.round((d.data.band + 1) * bandSize));
  ...
  const centerColor: string =
    overallColors[colorIndex] ?? overallColors[overallColors.length - 1] ?? '#6b7280';
