...
export interface ColorSchemesContext {
  getCategoryColors: (category: string, bands: number) => string[];
  getCategoryBaseColor: (category: string) => string;
  categoryColorSchemes: Ref<Record<string, string>>;
...
export const useColorSchemes = () => {
  ...
  const getSchemeColors = (schemeName: string, count: number, category?: string): string[] => {
    ...
    if (!schemeName.startsWith('interpolate')) {
      try {
        const discreteScheme: Record<number, string[]> | undefined = (
          d3 as Record<string, unknown>
        )[schemeName] as Record<number, string[]> | undefined;
    ...
    try {
      const interpolator = (d3 as Record<string, unknown>)[interpolateName] as
        | ((t: number) => string)
        | undefined;
